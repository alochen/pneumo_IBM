########## Code that defines parameters and sets up population & serotype rates dataframes ############
...
# import simulation number from batch file - comment out if running locally
simnum = sys.argv[1]
simnum = int(simnum)

params = pd.read_csv(r"//qdrive.dide.ic.ac.uk/homes/al817/Technical/Python/simparams.csv")
modparams = params.loc[np.where(params.simnum == simnum)[0]].reset_index(drop=True)

#modparams = pd.read_csv(r"//qdrive.dide.ic.ac.uk/homes/al817/Technical/Python/modparams.csv")
#country = modparams['country'].iloc[0]
...
seroparams = seroparams.drop(seroparams.index[seroparams.serotype.isin(['15B/C_old','15B','15C'])]).reset_index(drop=True)
...
ind_lambdas = carriers_all.copy(deep = True).astype(np.float32) # dataframe for individual serotype lambda rates 
#ind_lambdas = ind_lambdas.to_numpy(dtype = np.float16)
susc_full = max_carr_cap - carriers_all.sum(axis = 1) # susc = how many more serotypes can be carried?
...
# beta (transmission or FOI) rates ## these will be the same for all serotypes but could be changed in the future by agegrp
...
# assign individual serotype rates in dataframe
for s in range(indivs):
    for j in range(num_sero):
        ...
        # if person not carrying, serotype can switch at FOI rate
