...
IRRfun <- function(foldernam) {
  ...
  ## Pooling IRR using fixed effects model 
  meta_df <- list()
  dflen <- 1
...
#usa_agegrpstats_locinv <- read.csv('fra_agegrpstats_locinv.csv')
usa_glob_agegrpstats <- read.csv('usa_agegrpstats_glob.csv')
#usa_dis_agegrpstats <- read.csv('fra_agegrpstats_dis.csv')
...
# FRANCE ONLY
carrprevplot_fr <- function(vaccinperiod) {
  carrprev_t <- carrprev_avg %>% filter(vaxperiod == vaccinperiod)
  carrprev_t$vaxperiod <- factor(carrprev_t$vaxperiod,
                                   levels = c('prevacc', 'postPCV13', 'postPCV20', 'postPCV30'),
                                   labels = c('Pre-hPCV', 'Post-hPCV13', 'Post-hPCV20', 'Post-hPCV30'))
  ggplot(carrprev_t) + 
  geom_errorbar(aes(x = description, ymin = lower_ci*100, ymax = upper_ci*100, colour = agegrp), 
                width = 0.1, position = position_dodge(0.2)) + 
  geom_point(aes(x = description, y = smean*100, colour = agegrp), position = position_dodge(0.2)) + 
  labs(x = "", y = "Carriage prevalence (%)", colour = '') +
  theme_bw() + ggtitle(unique(carrprev_t$vaxperiod)) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) +
  scale_color_viridis(discrete = T)} # pdf 5x7

fra_carrprev_prePCVpl <- carrprevplot_fr('prevacc')
fra_carrprev_post13pl <- carrprevplot_fr('postPCV13')
fra_carrprev_post20pl <- carrprevplot_fr('postPCV20')
fra_carrprev_post30pl <- carrprevplot_fr('postPCV30')

ggarrange(fra_carrprev_prePCVpl, fra_carrprev_post13pl, fra_carrprev_post20pl, fra_carrprev_post30pl,
          nrow = 2, ncol = 2, common.legend = T, legend = 'bottom')
...
##### Average disease incidence (default scenario)
fra_disincidence <- fra_agegrpstats %>% filter(X == 'disease inc') %>% 
  pivot_longer(cols = c('overall', 'child', 'adult', 'elder'), 
               names_to = 'agegrp', values_to = 'diseaseinc')
fra_disincidence$X.1 <- NULL
fra_disincidence$X <- NULL

fra_avg_disinc <- fra_disincidence %>% filter(description == 'default5k') %>% 
  group_by(description, vaxperiod, vaxstrat, agegrp) %>% 
  summarise(smean = mean(diseaseinc, na.rm = T), 
            ssd = sd(diseaseinc, na.rm = T), 
            count = n()) %>%
  mutate(se = ssd/sqrt(count), 
         lower_ci = lower_ci(smean, se, count), 
         upper_ci = upper_ci(smean, se, count))
fra_avg_disinc$agegrp <- factor(fra_avg_disinc$agegrp, levels = c('overall', 'child', 'adult', 'elder'),
                                labels = c('All', 'Children', 'Adults', 'Elderly'))
fra_avg_disinc$vaxperiod <- factor(fra_avg_disinc$vaxperiod, 
                                   levels = c('prevacc', 'postPCV13', 'postPCV20', 'postPCV30'),
                                   labels = c('Pre-hPCV', 'Post-hPCV13', 'Post-hPCV20', 'Post-hPCV30'))

fra_avg_disinc_prevax <- fra_avg_disinc %>% filter(vaxperiod == 'Pre-hPCV')
fra_avg_disinc_prevax$vaxstrat <- NULL

fra_avgdisinc_disincstrat <- ggplot(fra_avg_disinc %>% filter(vaxstrat == 'dis_inc') %>% 
                                      full_join(fra_avg_disinc_prevax %>% mutate(vaxstrat = 'dis_inc')), 
                                    aes(x = vaxperiod, y = smean, colour = agegrp)) + 
  geom_point(position = position_dodge(width = 0.5)) + 
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), 
                width = 0.01, position = position_dodge(width = 0.5)) + 
  scale_color_viridis(discrete = T) + 
  theme_bw() + ggtitle('Disease incidence') + 
  labs(x = '', y = 'Avg disease incidence (per 100,000ppl)', colour = '') + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

fra_avgdisinc_globstrat <- ggplot(fra_avg_disinc %>% filter(vaxstrat == 'glob_inv') %>% 
                                      full_join(fra_avg_disinc_prevax %>% mutate(vaxstrat = 'glob_inv')), 
                                    aes(x = vaxperiod, y = smean, colour = agegrp)) + 
  geom_point(position = position_dodge(width = 0.5)) + 
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), 
                width = 0.01, position = position_dodge(width = 0.5)) + 
  scale_color_viridis(discrete = T) + 
  theme_bw() + ggtitle('Global invasiveness') + 
  labs(x = '', y = 'Avg disease incidence (per 100,000ppl)', colour = '')+ 
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

fra_avgdisinc_invstrat <- ggplot(fra_avg_disinc %>% filter(vaxstrat == 'local_inv'), 
                                    aes(x = vaxperiod, y = smean, colour = agegrp)) + 
  geom_point(position = position_dodge(width = 0.5)) + 
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), 
                width = 0.01, position = position_dodge(width = 0.5)) + 
  scale_color_viridis(discrete = T) + 
  theme_bw() + ggtitle('Local invasiveness') + 
  labs(x = '', y = 'Avg disease incidence (per 100,000ppl)', colour = '') + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

default_avgdisinc <- ggarrange(fra_avgdisinc_disincstrat, fra_avgdisinc_invstrat, fra_avgdisinc_globstrat,
          nrow = 1, ncol = 3, common.legend = T, legend = 'bottom')
ggsave(default_avgdisinc, file = 'Q:/Technical/Python/figs for paper/fig20_fra_default_avgdisinc.pdf', 
       height = 5, width = 9)


######### Comparison glob with USA ############
usa_glob_meta <- read.csv('usa_glob_meta.csv')
usa_glob_meta$X <- NULL
...
glob_meta$country <- factor(glob_meta$country, levels = c('fra','usa'),
                            labels = c('France', 'USA'))

glob_comp <- function(agegroup, vaccinperiod) {
  if (vaccinperiod == 'PCV13') {
    angl <- 48
  } else(angl <- 45)
  ggplot(glob_meta %>% filter(agegrp == agegroup, vaxperiod == vaccinperiod), 
         aes(x = description, y = fixed, colour = country)) +
    geom_hline(aes(yintercept = 0), linetype = 'dashed') + 
    geom_errorbar(aes(ymin = fixed.lo, ymax = fixed.hi), position = position_dodge(width = 0.2), width = 0.1) +
    geom_point(position = position_dodge(width = 0.2)) + labs(colour = "", x = '', y = 'log(Pooled IRR)') +
    scale_color_manual(values = c('#440154FF', '#21908CFF')) + ggtitle(agegroup) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = angl, hjust = 1))} # angle = 48 for PCV13

PCV13usa_vs_fr_irr_all <- glob_comp('All', 'PCV13')
PCV13usa_vs_fr_irr_ch <- glob_comp('Children', 'PCV13')
PCV13usa_vs_fr_irr_ad <- glob_comp('Adults', 'PCV13')
PCV13usa_vs_fr_irr_eld <- glob_comp('Elderly', 'PCV13')

usa_vs_fr_PCV13 <- ggarrange(PCV13usa_vs_fr_irr_all, PCV13usa_vs_fr_irr_ch, 
                             PCV13usa_vs_fr_irr_ad, PCV13usa_vs_fr_irr_eld,
                             nrow = 2, ncol = 2, common.legend = T, legend = 'bottom') # pdf 6x8
usa_vs_fr_PCV13 <- annotate_figure(usa_vs_fr_PCV13, top = text_grob("Post-hPCV13", face = "bold", size = 15))


PCV20usa_vs_fr_irr_all <- glob_comp('All', 'PCV20')
PCV20usa_vs_fr_irr_ch <- glob_comp('Children', 'PCV20')
PCV20usa_vs_fr_irr_ad <- glob_comp('Adults', 'PCV20')
PCV20usa_vs_fr_irr_eld <- glob_comp('Elderly', 'PCV20')

usa_vs_fr_PCV20 <- ggarrange(PCV20usa_vs_fr_irr_all, PCV20usa_vs_fr_irr_ch, 
                                PCV20usa_vs_fr_irr_ad, PCV20usa_vs_fr_irr_eld,
                                nrow = 2, ncol = 2, common.legend = T, legend = 'bottom') # pdf 6x8
usa_vs_fr_PCV20 <- annotate_figure(usa_vs_fr_PCV20, top = text_grob("Post-hPCV20", face = "bold", size = 15))

PCV30usa_vs_fr_irr_all <- glob_comp('All', 'PCV30')
PCV30usa_vs_fr_irr_ch <- glob_comp('Children', 'PCV30')
PCV30usa_vs_fr_irr_ad <- glob_comp('Adults', 'PCV30')
PCV30usa_vs_fr_irr_eld <- glob_comp('Elderly', 'PCV30')

usa_vs_fr_PCV30 <- ggarrange(PCV30usa_vs_fr_irr_all, PCV30usa_vs_fr_irr_ch, 
                                PCV30usa_vs_fr_irr_ad, PCV30usa_vs_fr_irr_eld,
                                nrow = 2, ncol = 2, common.legend = T, legend = 'bottom') # pdf 6x8
usa_vs_fr_PCV30 <- annotate_figure(usa_vs_fr_PCV30, top = text_grob("Post-hPCV30", face = "bold", size = 15))

ggsave(usa_vs_fr_PCV13, file = 'Q:/Technical/Python/figs for paper/fig22_USA_vs_FR_IRRPCV13.pdf', 
       height = 6, width = 8.1)
ggsave(usa_vs_fr_PCV20, file = 'Q:/Technical/Python/figs for paper/fig23_USA_vs_FR_IRRPCV20.pdf', 
       height = 6, width = 8.1)
ggsave(usa_vs_fr_PCV30, file = 'Q:/Technical/Python/figs for paper/fig24_USA_vs_FR_IRRPCV30.pdf', 
       height = 6, width = 8.1)

####### USA VT carr prev plot #########
