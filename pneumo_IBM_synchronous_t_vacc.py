...
sim_output2.columns = ['key', 'value']
seroprev2 = sim_output2.drop([0, 72])
seroprev2 = seroprev2.rename(columns = {'key': 'Serotype', 'value': 'carrprev'})
seroprev2['carrprev'] = seroprev2['carrprev']/1000

### Get disease incidence using invasiveness
disinc2 = seroprev2.merge(finland_inv, on = 'Serotype')
disease_cas2 = disinc2['carrprev']*disinc2['invasiveness']
disinc2['disease'] = disease_cas2*100000
# save to file destination:
#newfiledest = "//qdrive/homes/al817/Technical/Python/IBM sims/sim_diseaseincidence.csv"
#mergedf.to_csv(path_or_buf= newfiledest, index = True)

### Top 13 disease-causing serotypes get vaccinated against
top7add = np.array(disinc2.disease)[np.argsort(disinc2.disease)][-13:]
topadd_ind = np.isin(disinc2.disease, top7add)
new_vt = disinc2.loc[topadd_ind].Serotype
new_vt_ind = np.where(np.isin(colnames, new_vt))

vacc2_stop_t <- t + 7300 # current time + 20 years
...
newfiledest3 = "//qdrive/homes/al817/Technical/Python/output" + str(simnum) + "postvacc_extendedPCV.csv"
sim_output3.to_csv(path_or_buf= newfiledest3, index = True)

# prevcol
prevcolnam3 = "//qdrive/homes/al817/Technical/Python/prevcol" + str(simnum) + "postvacc_extendedPCV.csv"
prev_col.to_csv(path_or_buf = prevcolnam3, index = True)

# carriers
carrnam3 =  "//qdrive/homes/al817/Technical/Python/carriers_tmax" + str(simnum) + "postvacc_extendedPCV.csv"
carriers.to_csv(path_or_buf = carrnam3, index = True)

# output figure
plotname3 = "simulation" + str(simnum) + "prevacc.pdf"
