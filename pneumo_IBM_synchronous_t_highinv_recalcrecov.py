...
filedest = "//qdrive/homes/al817/Technical/Python/popruns/" + country + '_' + globalinv*("glob") + "inv/" 
...
#                                 'carr.prev', 'carr.prev.low', 'carr.prev.high', 'agegrp', 'vaccinationera'], axis = 1)
# 
...
if country == "fi":
    ...
    country_inv = pd.read_csv(r"//qdrive.dide.ic.ac.uk/homes/al817/Technical/Python/francelocalinv.csv")
...
# top 20 invasive serotypes get vaccinated against
...
if country == 'usa':
    ...
    maxage = round(78.5*365) # life expectancy USA 78.5 years (2010) https://datacatalog.worldbank.org/dataset/world-development-indicators 
if country == 'fi':
    contact_matrix = contact_matrix_fi
    maxage = round(79.9*365) # life expectancy 79.9 years (2010) https://datacatalog.worldbank.org/dataset/world-development-indicators 
else:
    contact_matrix = contact_matrix_fr
    maxage = round(81.7*365) # life expectancy 81.7 years (2010) https://datacatalog.worldbank.org/dataset/world-development-indicators 

age_breakdown = [730, 23725, maxage]
...
#while t <= stop_t and tot_lambda1 != 0:
    ...
    if len(np.where(carriers.age > maxage)[0]) > 0:
        ind_to_die = np.where(carriers.age > maxage)[0]
    ...
    for i in range(numindivspertime):
        ...
        if (new_susc < carriers.susc.loc[which_person]):
            if (carriers.age.loc[which_person] <= 23360): 
...
overallcarrprevchild = len(np.where(carriers[agegroup == 0].susc < max_carr_cap)[0])/pop_child
overallcarrprevadult = len(np.where(carriers[agegroup == 1].susc < max_carr_cap)[0])/pop_child
overallcarrprevelder = len(np.where(carriers[agegroup == 2].susc < max_carr_cap)[0])/pop_child
...
overallcarrprevchild2 = len(np.where(carriers[agegroup == 0].susc < max_carr_cap)[0])/pop_child
overallcarrprevadult2 = len(np.where(carriers[agegroup == 1].susc < max_carr_cap)[0])/pop_child
overallcarrprevelder2 = len(np.where(carriers[agegroup == 2].susc < max_carr_cap)[0])/pop_child
...
overallcarrprevchild3 = len(np.where(carriers[agegroup == 0].susc < max_carr_cap)[0])/pop_child
overallcarrprevadult3 = len(np.where(carriers[agegroup == 1].susc < max_carr_cap)[0])/pop_child
overallcarrprevelder3 = len(np.where(carriers[agegroup == 2].susc < max_carr_cap)[0])/pop_child
