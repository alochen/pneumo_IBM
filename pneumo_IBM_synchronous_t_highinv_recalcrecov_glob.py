...
# country_inv.to_csv(path_or_buf = "//qdrive/homes/al817/Technical/Python/francelocalinv.csv", index = False)
# country_inv.to_csv(path_or_buf = "//qdrive/homes/al817/Technical/Python/finlandlocalinv.csv", index = False)
# country_inv.to_csv(path_or_buf = "//qdrive/homes/al817/Technical/Python/usalocalinv.csv", index = False)
...
# new_vt_ind = np.where(np.isin(colnames, new_vt))[0]
# 
# # top 30 invasive serotypes get vaccinated against
# top10add = np.array(country_inv.invasiveness)[np.argsort(country_inv.invasiveness)][-30:-20]
# new_vt2 = np.array(country_inv.Serotype)[np.argsort(country_inv.invasiveness)][-30:-20]
# new_vt_ind2 = np.where(np.isin(colnames, new_vt2))[0]
...
#     new_vt_ind = np.where(np.isin(colnames, new_vt))[0]
#     top10add = np.array(global_inv.invasiveness)[np.argsort(global_inv.invasiveness)][-30:-20]
#     new_vt2 = np.array(global_inv.Serotype)[np.argsort(global_inv.invasiveness)][-30:-20]
#     new_vt_ind2 = np.where(np.isin(colnames, new_vt2))[0]
...
overalldis = ((disease_child + disease_adult + disease_elder).sum()/(indivs*(t/365)))*100000
